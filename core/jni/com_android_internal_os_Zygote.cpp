  ...
  */
  ...
  if (is_top_app) {
    if (use_fifo_ui) {
      ...
      // Set SCHED_RR for top-app fork where FIFO UI is disabled.
      struct sched_param rt_param;
      rt_param.sched_priority = 1; // 98 prio

      if (number_of_processors == 1) {
        RuntimeAbort(env, __LINE__, "Cannot set SCHED_RR on single-core devices.");
      }

      if (sched_setscheduler(0, SCHED_RR, &rt_param) != 0) {
        ALOGE("Failed to set SCHED_RR: %s", strerror(errno));
      } else {
        ALOGW("Setting SCHED_RR for top-app fork");
      }
    }
  ...
  if (pid == 0) {
    if (is_top_app) {
      if (use_fifo_ui) {
        ...
        // Explicitly set SCHED_RR | SCHED_RESET_ON_FORK for child's main thread to avoid other
        // threads inheriting the main thread's FIFO scheduling policy.
        // Empirically, SCHED_RESET_ON_FORK also applies to newly created threads.
        // An app shall have no more than the main and render threads running at
        // FIFO priority, thus not saturating the device with SCHED_FIFO tasks.
        struct sched_param child_rt_param = {0};
        child_rt_param.sched_priority = 1; // 98 prio
        if (sched_setscheduler(0, SCHED_RR | SCHED_RESET_ON_FORK, &child_rt_param) != 0) {
          ALOGE("Failed to set SCHED_RR | SCHED_RESET_ON_FORK: %s", strerror(errno));
          RuntimeAbort(env, __LINE__, "Failed to set SCHED_RR | SCHED_RESET_ON_FORK in child");
        }
      }
    } else if (is_priority_fork) {
        setpriority(PRIO_PROCESS, 0, PROCESS_PRIORITY_MAX);
